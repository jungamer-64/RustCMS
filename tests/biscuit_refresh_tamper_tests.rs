...
async fn build_db() -> Option<Database> {
    ...
    let cfg = DatabaseConfig { url, max_connections: 2, min_connections: 1, connection_timeout: 5, idle_timeout: 30, max_lifetime: 300, enable_migrations: false };
    Database::new(&cfg).await.ok()
}

fn dummy_user() -> User {
    let now = Utc::now();
    User { id: Uuid::new_v4(), username: format!("user_tamper_{}", Uuid::new_v4().simple()), email: format!("tamper+{}@example.com", Uuid::new_v4()), password_hash: Some("$argon2id$v=19$m=65536,t=3,p=4$YWJj$YWJj".into()), first_name: None, last_name: None, role: UserRole::Subscriber.as_str().to_string(), is_active: true, email_verified: false, last_login: None, created_at: now, updated_at: now }
}

async fn build_auth(db: &Database) -> AuthService {
    use biscuit_auth::KeyPair; use base64::Engine; let kp = KeyPair::new();
...
fn naive_tamper(b64: &str) -> String {
    ...
    if text.contains("refresh") { text = text.replace("refresh", "access"); }
...
async fn tampered_refresh_token_rejected() {
    let db = match build_db().await { Some(d) => d, None => { eprintln!("SKIP tampered_refresh_token_rejected: DATABASE_URL not set"); return; } };
...
async fn random_garbage_token_rejected() {
    let db = match build_db().await { Some(d) => d, None => { eprintln!("SKIP random_garbage_token_rejected: DATABASE_URL not set"); return; } };
    let auth = build_auth(&db).await;
    // 生成されていない乱数 base64 文字列
    let garbage = base64::engine::general_purpose::STANDARD.encode("totally-invalid-token");
    assert!(auth.refresh_access_token(&garbage).await.is_err(), "garbage token should be invalid");
}
