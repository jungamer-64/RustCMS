...
fn auth_success_serialization_contains_flat_fields() {
    let tokens = AuthTokens { access_token: "A".into(), refresh_token: "R".into(), biscuit_token: "B".into(), expires_in: 1234, session_id: "S".into() };
    let user = sample_user();
    let resp = AuthSuccessResponse::from_parts(&tokens, user);
    let v = serde_json::to_value(&resp).expect("serialize resp");
    let obj = v.as_object().unwrap();
    for k in ["access_token","refresh_token","biscuit_token","expires_in","session_id","token"] { assert!(obj.contains_key(k), "expected key `{k}` present with feature enabled"); }
}
...
fn auth_success_serialization_excludes_flat_fields() {
    ...
    let v = serde_json::to_value(&resp).expect("serialize resp");
    let obj = v.as_object().unwrap();
    for k in ["access_token","refresh_token","biscuit_token","expires_in","session_id","token"] { assert!(!obj.contains_key(k), "key `{k}` should be absent with feature disabled"); }
}
