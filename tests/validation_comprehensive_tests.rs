...
fn test_email_format_validation() {
    ...
    for email in invalid_emails {
        ...
        let is_invalid = email.is_empty()
            ...
            || email.starts_with('@')
            || email.ends_with('@');
        assert!(is_invalid, "Expected {email} to be invalid");
    }
}

#[test]
fn test_username_format_validation() {
    let valid_usernames = vec!["user123", "valid_user", "User-Name", "user.name"];
...
fn test_password_strength_requirements() {
    let weak_passwords = vec!["", "123", "password", "12345678", "abcdefgh"];

    let strong_passwords = vec!["P@ssw0rd123!", "MyStr0ng!Pass", "C0mplex#Password"];
...
fn test_path_traversal_prevention() {
    ...
    for path in malicious_paths {
        ...
        let has_traversal = path.contains("..") || path.starts_with('/') || path.contains("\\..\\");
...
fn test_unicode_normalization() {
    ...
    let unicode_strings = vec!["café", "日本語", "🚀", "Ñoño", "Москва"];
...
fn test_null_byte_injection() {
    ...
    let inputs_with_nulls = vec!["test\0null", "\0", "prefix\0suffix"];
...
fn test_large_number_handling() {
    ...
    let numbers = vec![i64::MIN, i64::MAX, 0_i64, -1_i64, 1_i64];
