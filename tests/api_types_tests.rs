...
use cms_backend::utils::{
    api_types::{ApiResponse, PaginatedResponse, PaginationQuery, Pagination},
    error::AppError,
};
use serde_json;
...
mod api_types_tests {
    ...
    fn test_api_response_success_with_message() {
        let response = ApiResponse::success_with_message("test data", "Success message".to_string());
        let json = serde_json::to_string(&response).unwrap();
    ...
    fn test_pagination_query_validation() {
        // Valid query
        let query: Result<PaginationQuery, _> = serde_json::from_str(r#"{"page": 2, "per_page": 20}"#);
        assert!(query.is_ok());
        
        // Test validation method
        let mut query = PaginationQuery { page: 0, per_page: 0 };
        query.validate();
        assert_eq!(query.page, 1);
        assert_eq!(query.per_page, 20);
    }
    ...
    fn test_app_error_http_status() {
        ...
        assert_eq!(AppError::Authentication("Test".to_string()).status_code(), 401);
        assert_eq!(AppError::Authorization("Test".to_string()).status_code(), 403);
        assert_eq!(AppError::InternalServer("Test".to_string()).status_code(), 500);
    }
