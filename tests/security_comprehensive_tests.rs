...
fn test_role_permissions_mapping() {
    ...
    let roles_with_expected_permissions = vec![
        (UserRole::SuperAdmin, vec!["admin", "manage_users", "manage_posts", "manage_settings"]),
...
fn test_content_security_policy_headers() {
    ...
    for directive in csp_directives {
        ...
        assert!(directive.contains("'self'") || directive.contains("data:") || directive.contains("https:"));
    }
}
