...
async fn test_user_created_event_flow() {
    ...
    assert!(ctx.search.verify_user_indexed(user.id).await, 
            "User should be indexed in search");
    
    // Verify cache invalidation occurred
    assert!(ctx.cache.verify_key_deleted(&format!("user:{}", user.id)).await, 
            "User cache should be invalidated");
    assert!(ctx.cache.verify_pattern_deleted("users:").await, 
...
async fn test_post_created_triggers_search_indexing() {
    ...
    // Verify search indexing occurred
    assert!(ctx.search.verify_post_indexed(post.id).await, 
    ...
    // Verify cache invalidation occurred
    assert!(ctx.cache.verify_key_deleted(&format!("post:{}", post.id)).await);
...
async fn test_multiple_listeners_receive_same_event() {
    ...
    assert!(ctx.cache.verify_key_deleted(&format!("user:{}", user_id)).await);
}
...
async fn test_listener_error_doesnt_crash_system() {
    ...
    assert_eq!(ctx.search.index_user_call_count().await, 1,
    ...
    assert!(ctx.cache.verify_key_deleted(&format!("user:{}", another_user.id)).await);
}
...
async fn test_listener_lag_handling() {
    ...
    let user_ids: Vec<Uuid> = (0..10).map(|i| {
    ...
    println!("Total events emitted: {}, Indexed: {}", user_ids.len(), indexed_count);
    
    // The listener should have processed at least SOME events (not 0, not all)
    assert!(indexed_count > 0, "Listener should have processed some events");
    assert!(indexed_count < user_ids.len(), 
