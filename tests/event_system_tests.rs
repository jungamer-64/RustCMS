...
async fn test_event_bus_creation() {
    ...
    assert_eq!(
        sender.len(),
        0,
        "New event bus should have no pending messages"
    );
}
...
async fn test_user_updated_event() {
    ...
    event_bus
        .send(AppEvent::UserUpdated(event_data.clone()))
        .ok();
...
async fn test_post_created_event() {
    ...
    event_bus
        .send(AppEvent::PostCreated(event_data.clone()))
        .ok();
...
async fn test_post_published_event() {
    ...
    event_bus
        .send(AppEvent::PostPublished(event_data.clone()))
        .ok();
...
async fn test_multiple_subscribers() {
    ...
    // Send one event
    event_bus
        .send(AppEvent::UserCreated(event_data.clone()))
        .ok();
