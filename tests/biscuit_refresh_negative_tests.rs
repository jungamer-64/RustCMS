...
fn dummy_user() -> User {
    let now = Utc::now();
    User { id: Uuid::new_v4(), username: format!("user_neg_{}", Uuid::new_v4().simple()), email: format!("neg+{}@example.com", Uuid::new_v4()), password_hash: Some("$argon2id$v=19$m=65536,t=3,p=4$YWJj$YWJj".into()), first_name: None, last_name: None, role: UserRole::Subscriber.as_str().to_string(), is_active: true, email_verified: false, last_login: None, created_at: now, updated_at: now }
}
...
async fn refresh_fails_after_expiry() {
    let db = match build_db().await { Some(d) => d, None => { eprintln!("SKIP refresh_fails_after_expiry: DATABASE_URL not set"); return; } };
...
async fn refresh_version_mismatch_reuse_old() {
    let db = match build_db().await { Some(d) => d, None => { eprintln!("SKIP refresh_version_mismatch_reuse_old: DATABASE_URL not set"); return; } };
