...
fn api_key_generation_and_verify() {
    ...
    assert!(model.verify_key(&raw).expect("verification should not fail"));
    assert!(!raw.is_empty());
...
fn api_key_verify_wrong() {
    ...
    let user_id = Uuid::new_v4();
    let (model, _raw) =
        ApiKey::new_validated("test-key".to_string(), user_id, vec!["posts:read".into()]).expect("should create a valid API key");
    assert!(!model.verify_key("invalid").expect("verification should not fail"));
