...
use domain::comment::{Comment, CommentId, CommentStatus, CommentText};
use domain::post::{Content, Post, PostId, Slug};
...
impl DieselUserRepository {
    ...
    fn db_to_domain(db_user: DbUser) -> Result<User, RepositoryError> {
        ...
        // Parse role
        let role = domain::user::UserRole::from_str(&db_user.role)
...
impl DieselPostRepository {
    ...
    fn db_to_domain(db_post: DbPost) -> Result<Post, RepositoryError> {
        ...
        let title = domain::post::Title::new(db_post.title)
        ...
        // Parse status
        let status = domain::post::PostStatus::from_str(&db_post.status)
