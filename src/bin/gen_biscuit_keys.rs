...
fn validate_path_length(path: &Path) -> cms_backend::Result<()> {
    ...
    if path_str.len() > MAX_PATH_LENGTH {
        return Err(cms_backend::AppError::BadRequest(format!(
            "Path too long (max {MAX_PATH_LENGTH} chars): {path_str}"
        )));
    }
...
fn handle_list_command(args: &Args) {
...
fn backup_if_exists(path: &Path, args: &Args) -> cms_backend::Result<()> {
    ...
    let original_size = fs::metadata(path).map(|m| m.len()).map_err(|e| {
        cms_backend::AppError::Internal(format!(
    ...
    .map_err(|e| {
        cms_backend::AppError::Internal(format!("Backup failed for {}: {}", path.display(), e))
    })?;
    ...
    match fs::metadata(path) {
        Ok(meta) => {
            if meta.len() != original_size {
                warn!(
                    "File size changed during backup: {} (was {}, now {})",
                    path.display(),
                    original_size,
                    meta.len()
                );
        ...
        Err(e) => {
            ...
            return Err(cms_backend::AppError::Internal(format!(
                "Original file lost during backup of {}: {}",
                path.display(),
                e
            )));
        }
...
fn write_env_file(
    ...
    append_or_update_env(file, priv_b64, pub_b64, args.force).map_err(|e| {
        cms_backend::AppError::Internal(format!("Failed to update {}: {}", file.display(), e))
    })?;
...
fn create_new_env_file(path: &Path, priv_b64: &str, pub_b64: &str) -> std::io::Result<()> {
...
fn append_env_keys(path: &Path, priv_b64: &str, pub_b64: &str) -> std::io::Result<()> {
...
fn append_or_update_env(
    ...
    let has_keys =
        content.contains("BISCUIT_PRIVATE_KEY_B64=") || content.contains("BISCUIT_PUBLIC_KEY_B64=");
