//! 投稿公開ユースケース（PublishPostUseCase）
//!
//! # 責務
//! - 投稿を Draft から Published 状態に遷移させる
//! - Repository を使って永続化
//! - AppEvent::PostPublished イベントを発行
//!
//! # ビジネスルール
//! - 投稿は Draft 状態である必要がある（Domain層で検証）
//! - published_at タイムスタンプを自動設定
//!
//! # 使用例
//! ```ignore
//! let use_case = PublishPostUseCase::new(post_repo, event_bus);
...
impl PublishPostUseCase {
    ...
    pub async fn execute(&self, post_id_str: &str) -> ApplicationResult<PostDto> {
        ...
        // 3. Domain Entity の publish メソッド呼び出し（状態検証含む）
        post.publish()
        ...
        // 6. PostPublished イベント発行
        let event = AppEvent::PostPublished(PostEventData {
            id: *post.id().as_uuid(),
...
mod tests {
    ...
    async fn test_publish_post_success() {
        ...
        let use_case = PublishPostUseCase::new(Arc::new(mock_repo), event_bus);
    ...
    #[tokio::test]
    async fn test_publish_post_already_published() {
        // Arrange
    ...
    #[tokio::test]
    async fn test_publish_post_invalid_uuid() {
        // Arrange
        let mut mock_repo = MockPostRepository::new();
