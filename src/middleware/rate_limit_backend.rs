...
#[cfg(feature = "monitoring")] use metrics::{gauge};
use std::sync::Once;
use tokio::time::interval;
#[cfg(feature = "cache")] use redis::AsyncCommands;
...
impl InMemoryRateLimiter {
    pub fn from_env() -> Self {
        ...
        let max_tracked = std::env::var("API_KEY_FAIL_MAX_TRACKED").ok().and_then(|v| v.parse::<usize>().ok()).unwrap_or(5000);
        let disabled = std::env::var("API_KEY_FAIL_DISABLE").map(|v| v=="1" || v.eq_ignore_ascii_case("true")).unwrap_or(false);
        let inst = Self { window: Duration::from_secs(window), threshold, max_tracked, disabled, map: Mutex::new(HashMap::new()), cleaner_started: Once::new() };
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    if map.len() > this.max_tracked {
                        let mut entries: Vec<_> = map.iter().map(|(k, (_c, ts))| (k.clone(), *ts)).collect();
                        entries.sort_by_key(|(_, ts)| *ts); // oldest first
                        for (k, _) in entries.into_iter().take(map.len() - this.max_tracked) { map.remove(&k); }
                    }
                    #[cfg(feature = "monitoring")] {
                        if _before != map.len() {
...
impl ApiKeyRateLimiter for InMemoryRateLimiter {
    ...
    self.spawn_background_cleaner();
        ...
        if map.len() >= self.max_tracked { // drop oldest
            if let Some(old) = map.iter().min_by_key(|(_, (_c, ts))| *ts).map(|(k, _)| k.clone()) { map.remove(&old); }
        }
    ...
    fn clear(&self, key: &str) { let mut map = self.map.lock(); map.remove(key); }
    fn tracked_len(&self) -> usize { self.map.lock().len() }
}

pub static GLOBAL_RATE_LIMITER: Lazy<InMemoryRateLimiter> = Lazy::new(|| InMemoryRateLimiter::from_env());
...
pub fn get_rate_limiter() -> &'static dyn ApiKeyRateLimiter {
    ...
    static BACKEND: Lazy<String> = Lazy::new(|| std::env::var("API_KEY_FAIL_BACKEND").unwrap_or_else(|_| "memory".into()));
    if BACKEND.as_str() == "redis" {
        #[cfg(feature = "cache")]
        {
            return REDIS_RATE_LIMITER.get_or_init(|| RedisRateLimiter::from_env()).as_dyn();
        }
...
impl ApiKeyRateLimiter for RedisRateLimiter {
        ...
        let result = tokio::task::block_in_place(|| {
            ...
            rt.block_on(async {
                if let Ok(mut conn) = self.client.get_multiplexed_async_connection().await {                    
                    let cnt: i64 = match conn.incr(&k, 1).await { Ok(v) => v, Err(_) => return false }; // on error do not block
                    if cnt == 1 { let _ : Result<(), _> = conn.expire(&k, window_secs as i64).await; }
    ...
    fn clear(&self, key: &str) {
        ...
        let _ = tokio::task::block_in_place(|| {
            let rt = tokio::runtime::Handle::current();
            rt.block_on(async {
                if let Ok(mut conn) = self.client.get_multiplexed_async_connection().await { let _: Result<(), _> = conn.del(&k).await; }
            })
        });
    }
    fn tracked_len(&self) -> usize {
        ...
        if last.elapsed() < Duration::from_secs(15) { return cached; }
        // refresh
        let new_len = tokio::task::block_in_place(|| {
            let rt = tokio::runtime::Handle::current();
            rt.block_on(async {
                if let Ok(mut conn) = self.client.get_multiplexed_async_connection().await {
                    let mut cursor: u64 = 0; let mut total = 0usize;
                    loop {
                        let pattern = format!("{}*", self.key_prefix);
                        let res: redis::RedisResult<(u64, Vec<String>)> = redis::cmd("SCAN").arg(cursor).arg("MATCH").arg(&pattern).arg("COUNT").arg(100usize).query_async(&mut conn).await;
                        match res { Ok((next, keys)) => { total += keys.len(); if next == 0 { break total } else { cursor = next; } }, Err(_) => break 0 };
                    }
                } else { 0 }
            })
        });
