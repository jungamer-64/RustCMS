...
impl FixedWindowLimiter {
    ...
    pub fn allow(&self, key: &str) -> bool {
        ...
        if map.len() > 1024 { // only bother when moderately large
            let win = self.window;
    ...
    pub fn tracked_len(&self) -> usize { self.entries.lock().len() }
    pub fn window_secs(&self) -> u64 { self.window.as_secs() }
    pub fn max_requests(&self) -> u32 { self.max_requests }
}
...
pub trait GenericRateLimiter: Send + Sync {
    ...
    fn tracked_len(&self) -> usize { 0 }
    fn name(&self) -> &'static str { "generic" }
}

impl GenericRateLimiter for FixedWindowLimiter {
    fn check(&self, key: &str) -> RateLimitDecision {
        if self.allow(key) { RateLimitDecision::Allowed } else { RateLimitDecision::Blocked { retry_after_secs: self.window_secs() } }
    }
    fn tracked_len(&self) -> usize { self.tracked_len() }
    fn name(&self) -> &'static str { "ip_fixed_window" }
}
    ...
    impl GenericRateLimiter for ApiKeyFailureLimiterAdapter {
        fn check(&self, key: &str) -> RateLimitDecision {
            ...
            if backend.record_failure(key) {
                RateLimitDecision::Blocked { retry_after_secs: self.window_secs }
            } else { RateLimitDecision::Allowed }
        }
        fn clear(&self, key: &str) { get_rate_limiter().clear(key); }
        fn tracked_len(&self) -> usize { get_rate_limiter().tracked_len() }
        fn name(&self) -> &'static str { "api_key_failure_window" }
    }
}
