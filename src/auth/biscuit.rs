...
pub(super) fn build_token(
    ...
    .map_err(|e| AuthError::BiscuitError(format!("Failed to generate biscuit facts: {e}")))?;
    writeln!(&mut program, "token_type(\"{token_type}\");")
        .map_err(|e| AuthError::BiscuitError(format!("Failed to write token_type: {e}")))?;
    writeln!(&mut program, "exp({exp_unix});")
        .map_err(|e| AuthError::BiscuitError(format!("Failed to write exp: {e}")))?;
    writeln!(
        &mut program,
        "session(\"{}\", {version});",
        session_id.as_ref()
    )
    .map_err(|e| AuthError::BiscuitError(format!("Failed to write session: {e}")))?;

    let builder: BiscuitBuilder = biscuit_auth::Biscuit::builder();
    let builder = builder
        .code(&program)
        .map_err(|e| AuthError::BiscuitError(format!("Failed to build biscuit facts: {e}")))?;

    let token = builder
        .build(keypair)
        .map_err(|e| AuthError::BiscuitError(format!("Failed to sign biscuit: {e}")))?;
...
pub(super) fn parse_refresh_biscuit(token: &str, public_key: &PublicKey) -> Result<ParsedBiscuit> {
    ...
    if parsed.token_type != "refresh" {
        return Err(AuthError::TokenTypeMismatch {
            ...
            actual: parsed.token_type.clone(),
        }
        .into());
    }
    Ok(parsed)
...
fn create_authorizer(token: &str, public_key: &PublicKey) -> Result<biscuit_auth::Authorizer> {
    let unverified = biscuit_auth::UnverifiedBiscuit::from_base64(token)
        .map_err(|e| AuthError::BiscuitError(format!("Failed to parse biscuit token: {e}")))?;
    ...
    let biscuit = unverified.verify(key_provider).map_err(|e| {
        AuthError::BiscuitError(format!("Biscuit signature verification failed: {e}"))
    })?;
    let mut authorizer = biscuit
        .authorizer()
        .map_err(|e| AuthError::BiscuitError(format!("Failed to create authorizer: {e}")))?;
    authorizer
        .authorize()
        .map_err(|e| AuthError::BiscuitError(format!("Authorizer run failed: {e}")))?;
    Ok(authorizer)
}
...
fn get_user(authorizer: &mut biscuit_auth::Authorizer) -> Result<(Uuid, String, UserRole)> {
    ...
    let user_id = Uuid::parse_str(&id_s).map_err(|_| AuthError::InvalidTokenFormat)?;
    let role = UserRole::from_str(&role_s).map_err(|_| AuthError::InvalidTokenFormat)?;
...
fn get_session(authorizer: &mut biscuit_auth::Authorizer) -> Result<(SessionId, u32)> {
    ...
    let (sid, ver_i) = v.into_iter().next().ok_or(AuthError::InvalidTokenFormat)?;
    let ver_u32 = u32::try_from(ver_i).map_err(|_| AuthError::InvalidTokenFormat)?;
...
fn query_triple(
    ...
    v.into_iter()
        .next()
        .ok_or_else(|| AuthError::InvalidTokenFormat.into())
}
