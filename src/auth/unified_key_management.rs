...
use biscuit_auth::{KeyPair as BiscuitKeyPair, PrivateKey as BiscuitPrivateKey, PublicKey as BiscuitPublicKey};
...
impl UnifiedKeyPair {
    ...
    pub fn generate() -> Result<Self, AuthError> {
        ...
        // Biscuit鍵ペアも同じシードから生成
        let biscuit_private = BiscuitPrivateKey::from_bytes(&secret_bytes, BiscuitAlgorithm::Ed25519)
            .map_err(|e| AuthError::Biscuit(format!("Failed to create Biscuit private key: {e}")))?;
    ...
    fn try_load_from_env() -> Result<Option<Self>, AuthError> {
        ...
        let bytes = BASE64.decode(key_b64.trim())
            .map_err(|e| AuthError::Biscuit(format!("Invalid base64 in ED25519_PRIVATE_KEY_B64: {e}")))?;
        
        if bytes.len() != 32 {
            return Err(AuthError::Biscuit(
                format!("Invalid key length in ED25519_PRIVATE_KEY_B64: expected 32, got {}", bytes.len())
            ));
        }
        
        let signing_key = SigningKey::from_bytes(
            bytes.as_slice().try_into()
                .map_err(|_| AuthError::Biscuit("Failed to parse signing key".to_string()))?
        );
        let verifying_key = signing_key.verifying_key();
        
        let biscuit_private = BiscuitPrivateKey::from_bytes(&bytes, BiscuitAlgorithm::Ed25519)
            .map_err(|e| AuthError::Biscuit(format!("Failed to create Biscuit key: {e}")))?;
        let biscuit_keypair = Arc::new(BiscuitKeyPair::from(&biscuit_private));
    ...
    fn load_from_file(path: &str) -> Result<Self, AuthError> {
        let data = fs::read_to_string(path)
            .map_err(|e| AuthError::Biscuit(format!("Failed to read key file {}: {}", path, e)))?;
        
        let bytes = BASE64.decode(data.trim())
            .map_err(|e| AuthError::Biscuit(format!("Invalid base64 in key file: {e}")))?;
        
        if bytes.len() != 32 {
            return Err(AuthError::Biscuit(
                format!("Invalid key length in file: expected 32, got {}", bytes.len())
            ));
        }
    ...
    fn save_to_file(&self, path: &str) -> Result<(), AuthError> {
        ...
        if let Some(parent) = Path::new(path).parent() {
            fs::create_dir_all(parent)
                .map_err(|e| AuthError::Biscuit(format!("Failed to create key directory: {e}")))?;
        }
        
        let encoded = BASE64.encode(self.signing_key.to_bytes());
        fs::write(path, encoded)
            .map_err(|e| AuthError::Biscuit(format!("Failed to write key file: {e}")))?;
        ...
        #[cfg(unix)]
        {
            ...
            let mut perms = fs::metadata(path)
                .map_err(|e| AuthError::Biscuit(format!("Failed to read file metadata: {e}")))?
                .permissions();
            perms.set_mode(0o600);
            fs::set_permissions(path, perms)
                .map_err(|e| AuthError::Biscuit(format!("Failed to set file permissions: {e}")))?;
        }
    ...
    pub fn load_or_generate(config: &KeyLoadConfig) -> Result<Self, AuthError> {
        ...
        if config.is_production {
            ...
            return Err(AuthError::Biscuit(
                "Ed25519 key must be provided in production (via environment variable or file)".to_string()
            ));
        }
