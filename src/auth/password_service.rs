...
impl PasswordService {
    ...
    pub fn verify_password(&self, password: &str, hash: &str) -> Result<(), AuthError> {
        ...
        let parsed_hash = match PasswordHash::new(hash) {
            ...
            Err(e) => {
                ...
                // タイミング攻撃対策: ダミーの処理を実行
                let _ = self.argon2.hash_password(password.as_bytes(), &SaltString::generate(&mut OsRng));
    ...
    pub fn calculate_password_strength(&self, password: &str) -> u8 {
        ...
        let has_common_pattern = common_patterns.iter()
...
mod tests {
    ...
    fn test_password_strength() {
        ...
        assert!(weak_score < 40, "Expected weak score < 40, got {}", weak_score);
        
        // 中程度のパスワード
        let medium_score = service.calculate_password_strength("Password123");
        println!("medium_score (Password123): {}", medium_score);
        assert!(medium_score >= 30 && medium_score < 70, "Expected medium score 30-70, got {}", medium_score);
        ...
        assert!(strong_score >= 70, "Expected strong score >= 70, got {}", strong_score);
    }
    ...
    fn test_timing_attack_resistance() {
        ...
        println!("Timing: valid={:?}, invalid={:?}, diff={:?}", elapsed1, elapsed2, diff);
